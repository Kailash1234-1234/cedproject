...
class Brand implements ResolverInterface
{
    ...
    /**
     * @var brandpostFactory
     */

    private $brandpostFactory;
    ...
    public function resolve(Field $field, $context, ResolveInfo $info, array $value = null, array $args = null)  {
        ...
        if (!isset($args['brand_id'])) {
            throw new GraphQlAuthorizationException(
                __(
                    'brand_id for brand should be specified',
                )
            );
        }
        try {
            $data = $this->getBrandData($args['brand_id']);
     ...
     */
    private function getBrandData($brandId) : array
    {
        try {
            $customerData = [];
            $customerColl = $this->brandpostFactory->create()->getCollection()->addFieldToFilter('brand_id', ['eq'=>$brandId]);
